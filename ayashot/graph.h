...
struct Node{
    ...
    Node():UUID(__aya_random.next_ullong()){
    }
    Node(int _id):id(_id),UUID(__aya_random.next_ullong()){
...
struct Edge{
    ...
    int from , to ,id;
    Edge():UUID(__aya_random.next_ullong()){}
    Edge(int _id,int _from,int _to):
        id(_id),from(_from),to(_to),UUID(__aya_random.next_ullong()){
        __aya_map_id[UUID]=_id;
...
class Graph{
    ...
    public:
        ...
        void clain(int n , bool directed = true){
            int start_nodes = node_size + 1;
            int start_edges = edge_size + 1;
            tree_nodes(n),edge_size += n - 1;
            up(0,n - 1,i){
                E.push_back(Edge(start_edges + i,start_nodes + i,start_nodes + i + 1));
                F.push_back(Edge(start_edges + i,start_nodes + i + 1,start_nodes + i));
            }
            if(!directed){
                edge_size += n - 1 ,start_edges += n - 1;
                up(0,n - 1,i){
                    E.push_back(Edge(start_edges + i,start_nodes + i + 1,start_nodes + i));
                    F.push_back(Edge(start_edges + i,start_nodes + i,start_nodes + i + 1));
                }
            }
        }
        ...
        void tree_balanced(int n , bool directed = true){
            ...
            up(1,n - 1,i){
                E.push_back(Edge(start_edges + i, start_nodes + i,start_nodes + __aya_random.next(i) - 1));
            }
        }
