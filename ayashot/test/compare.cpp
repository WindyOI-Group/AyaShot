...
class Comparator{
	private:
		...
		bool open(string outf, string ansf){
	...
	public:
		bool compare_bits(string outf, string ansf){
			if(!open(outf, ansf)){
				//log here.
				return false;
			}
		...
		bool compare_noip(string outf, string ansf){
			if(!open(outf, ansf)){
				//log here.
				return false;
			}
...
int main(){
	Comparator CP;
	printf("%d",CP.compare_noip("test.out","test.ans"));
	return 0;
}